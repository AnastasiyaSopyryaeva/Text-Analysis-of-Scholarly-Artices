...
data = []
with open('social.jsonl', 'r', encoding='utf-8') as f:
    for line in f:
...
df_social = pd.DataFrame(filtered_data)

# Removing irrelevant for the study columns
df_social = df_social[['id', 'isPartOf', 'keyphrase', 'pageCount', 'publicationYear', 'publisher', 'title', 'wordCount', 'unigramCount', 'bigramCount', 'trigramCount', 'abstract', 'creator']]
...
stop_list_social = ['rates', 'heterosexual', 'process', 'without', 'feel', 'described', 'still', 'perceived', 'theory', 'others', 'terms', 'positive', 'negative', 'greater', 'survey', 'included', 'analysis', 'three', 'variables', 'respondents', 'population', 'including', 'number', 'substance', 'whether', 'important', 'model', 'sexually', 'current', 'findings', 'significant', 'general', 'toward', 'prevalence', 'based', 'compared', 'groups', 'years', 'report', 'attitudes', 'high', 'using', 'factors', 'related', 'table', 'individuals', 'associated', 'differences', 'higher', 'sample', 'scale', 'across', 'previous', 'total', 'potential', 'present', 'similar', 'provide', 'de', 'practice', 'increased', 'effects', 'given', 'levels', 'knowledge', 'identified', 'level', 'role', 'likely', 'review', 'significantly', 'effect', 'evidence', 'past' 'group', 'participants', 'results', 'et', 'c', 'b', 'e', 'n', 'p', 'r', 'j', 'homosexuality', 'reported', 'university', 'journal', 'issues', 'new', 'used', 'studies', 'percent', 'become', 'make', 'good', 'think', 'case', 'need', 'less', 'view', 'order', 'found', 'understanding', 'another', 'first', 'part', 'might', 'made', 'much', 'like', 'often', 'must', 'us', 'well', 'could', 'issue', 'human', 'see', 'may', 'even', 'people', 'would', 'also', 'although', 'many', 'orientation', 'author', 'essay', 'sexual', 'gay', 'gays', 'use', 'since', 'social', 'sexuality', 'article', 'within', 'lesbian', 'lesbians', 'study', 'research', 'among', 'homosexual', 'rather', 'thereby', 'paper', 'specific', 'fully', 'yet', 'al','one', 'two', 'data']
processed_unigrams_list = []
for i in unigrams_list:
    for key, value in i.items():
        key = key.lower()
        if key not in stopwords.words('english') and key not in stop_list_social and key.isalpha():       
...
num_topics = 7 # Check the number of topics
...
print('\nCoherence Score: ', coherence_lda)   # num_topics = 2, score = -0.3548; 
                                              # num_topics = 3, score = -0.3277; 
                                              # num_topics = 4, score = -0.4397; 
                                              # num_topics = 5, score = -0.4819, -0.4353; 
                                              # num_topics = 6, score = -0.4580;
                                              # num_topics = 7, score = -0.5470, -0.5300;
                                              # num_topics = 8, score = -0.5243
                                              # num_topics = 10, score = -0.6030, -0.5631
...
pyLDAvis.save_html(visualisation, 'LDA_Visualization_s13.2.html')

# 3) Analysing publishing dates distribution
df_social.groupby(['publicationYear'])['id'].agg('count').plot.bar(title='Documents by year (social science articles)', figsize=(20, 5), fontsize=12); 
plt.show()

# 4) Analysing page length distribution
plt.boxplot(x=df_social['pageCount'])
plt.suptitle('Page length (social articles)')
plt.show()

# 5) Analysing topic evolution by publication year 

df_social_year_less96 = df_social[df_social['publicationYear'] <= 1996]
df_social_year_less03 = df_social.query('publicationYear > 1996 and publicationYear <= 2003')
df_social_year_less08 = df_social.query('publicationYear > 2003 and publicationYear <= 2008')
df_social_year_less16 = df_social.query('publicationYear > 2008 and publicationYear <= 2016')
df_social_year_less21 = df_social[df_social['publicationYear'] > 2016]

unigrams_list = []
for i, v in df_social_year_less21.iterrows(): # change dataframe
